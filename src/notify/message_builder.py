...
from src.models import AnnouncementEntry, FileEntry, GradeChange
...
def build_announcement_message(new_announcements: list[AnnouncementEntry]) -> tuple[str, str]:
    """Build Bark title/body for new announcements."""
    count = len(new_announcements)
    title = f"Ninova: {count} new announcement{'s' if count != 1 else ''}"

    grouped: dict[str, list[AnnouncementEntry]] = defaultdict(list)
    for a in new_announcements:
        grouped[a.class_name].append(a)

    lines: list[str] = []
    for class_name, anns in grouped.items():
        lines.append(f"[{class_name}]")
        for a in anns:
            date_suffix = f" ({a.posted_at})" if a.posted_at else ""
            lines.append(f"  - {a.title}{date_suffix}")
